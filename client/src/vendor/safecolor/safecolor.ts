...
class SafeColor {
   ...
   */
  constructor(options?: { color: Array<number>; contrast: number }) {
   ...
   */
  calValidLumaRange(fgColor: Array<number>, contrastRatio: number) {
   ...
   * @param {Array} color
   */
  launderColor(color: Array<number>): Array<number> {
  ...
  random(str: string): string {
    ...
    let genColor: Array<number>;
    if (this.minLuma === this.maxLuma) {
      genColor = this.minLuma ? [255, 255, 255] : [0, 0, 0];
    } else if (!str) {
      genColor = [
      ...
      genColor = [0, 0, 0];
      for (let i = 0; i < genColor.length; ++i) {
        genColor[i] = (this.hash >> (i * 8)) & 255;
      }
    }
    genColor = this.launderColor(genColor);
    return 'rgb(' + genColor + ')';
  }
}
